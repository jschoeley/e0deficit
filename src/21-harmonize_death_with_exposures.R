...
paths$input <- list(
  ...
  pclm_log.txt = './tmp/21-pclm_log.txt',

  # skeleton path
  harmonized_skeleton.rds = './tmp/10-harmonized_skeleton.rds',

  # path to raw stmf death data
  stmf.rds = './dat/stmf/12-stmf.rds',
  # path to raw ons death data
  ons.rds = './dat/ons/12-ons_annual_deaths.rds',
  # path to raw cdc death data
...
paths$output <- list(
  # harmonized deaths
  harmonized_deaths.rds = './tmp/21-harmonized_death.rds',
  death_pclm.pdf = './tmp/21-death_pclm.pdf',
  hazard_pclm.pdf = './tmp/21-hazard_pclm.pdf',
  pclm_death_ungroup_diagnostics.pdf = './tmp/21-pclm_death_ungroup_diagnostics.pdf'
)
...
# Cut a vector and sum elements of cuts
# cuts = c(a, b, c, d) -> [a, b), [b, c), [c, d), [d, Inf)
CutSumVector <- function (x, cuts) {
  splits <- rep(cuts, times = c(diff(cuts), length(x)-max(cuts)+1))
  sapply(split(x, splits), sum)
}
...
# stmf death counts
dat$stmf <- readRDS(paths$input$stmf.rds)
# ons death counts
dat$ons <- readRDS(paths$input$ons.rds)
# cdc death counts
...
dat$cdc_ready_for_ungroup <-
  dat$cdc |>
  filter(
    !is.na(Year), !is.na(Gender), !is.na(`Single-Year Ages`),
    `Single-Year Ages` != 'Not Stated', Year > 1999, Year < 2020
  ) |>
  select(
...
dat$ungrouped_by_unique_age_pattern <-
  ...
  group_modify(~{
    ...
    # the variable <midyearpop> holds the population numbers
    # for the default analysis which may be a combination of
    # different sources depending on the country. for the
    # purpose of sensitivity analysis we also distinguish
    # <midyearpop_wpp22> and <midyearpop_wpp24>
    midyearpop <-
    ...
    population_py <- MidyearPop2Exposures(midyearpop$population_midyear, nweeksobserved)
    population_py_wpp22 <- MidyearPop2Exposures(midyearpop$population_midyear_wpp22, nweeksobserved)
    population_py_wpp24 <- MidyearPop2Exposures(midyearpop$population_midyear_wpp24, nweeksobserved)
    population_py_hmd <- MidyearPop2Exposures(midyearpop$population_midyear_hmd, nweeksobserved)

    # offset to be used for pclm
    # improves ungrouping by turning it
    # into a problem of estimating a smooth hazard
    pclm_offset <- CutSumVector(population_py, .x$age_start+1)
    ...
    ungrouped_deaths <- tryCatch({
      ...
      fit_pclm <-
        pclm(
          y = .x$deaths, x = .x$age_start, offset = pclm_offset,
          nlast = nlast, out.step = 1
        )
      ungrouped <- fitted.values(fit_pclm)[1:100]*population_py[1:100]
      # deaths ages 100+ via life table closure
      lx_100p <- c(1,cumprod(exp(-fitted.values(fit_pclm)[101:111])))
      dx_100p <- c(-diff(lx_100p), lx_100p[length(lx_100p)])
      infdx <- sum(dx_100p)
      infLx <- sum(lx_100p)
      infmx <- infdx/infLx
      ungrouped[101] <- sum(infmx*population_py[101])
      #sum(fitted.values(fit_pclm)[101:111])

      # PCLM return
      ungrouped <- tibble(
        age_start = 0:100,
        deaths = round(ungrouped, 2),
        population_py = round(population_py, 2),
        population_py_wpp22 = round(population_py_wpp22, 2),
        population_py_wpp24 = round(population_py_wpp24, 2),
        population_py_hmd = round(population_py_hmd, 2),
...
dat$ungrouped <-
  ...
  summarise(
    ...
    population_py_wpp22 = sum(population_py_wpp22),
    population_py_wpp24 = sum(population_py_wpp24),
    population_py_hmd = sum(population_py_hmd),
...
fig$death_pclm <-
  ...
  geom_line(
    aes(
      x = age_start, y = death, color = year,
      group = interaction(year), linewidth = is2020
    )
  ) +
  scale_linewidth_manual(values = c(0.1, 0.3)) +
  ...
  MyGGplotTheme() +
  labs(
    title = 'Ungrouped male death counts by age and country 2000-2024',
    subtitle = 'Dark lines more recent',
    x = '',
    y = ''
  )
fig$death_pclm
...
saveRDS(dat$death, file = paths$output$harmonized_deaths.rds)

ExportFigure(
  fig$death_pclm, filename = paths$output$death_pclm.pdf,
  scale = 1.5
)
ExportFigure(
  fig$hazard_pclm, filename = paths$output$hazard_pclm.pdf,
  scale = 1.5
)

ggsave(
  filename = paths$output$pclm_death_ungroup_diagnostics.pdf,
